...
export function UserProfilePage({
  ...
  const [purchasedSignals, setPurchasedSignals] = useState<BaseSignal[]>([]);
  ...
  async function loadUserData() {
    ...
    try {
      ...
      // Load purchased signals details
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-muted-foreground text-sm mb-2">
              {user.followerCount.toLocaleString("en-US")} followers on Farcaster
            </p>
            ...
            <Card className="p-4 bg-card border-primary/30">
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div className="text-center">
                  <div className="text-[#00ffcc] text-2xl mb-1">
                    {stats.successRate}%
                  </div>
                ...
                <div className="text-center">
                  <div className="text-foreground text-2xl mb-1">
              ...
              {isLoading ? (
                ...
                purchases.map((purchase) => (
                  <Card
                    ...
                    onClick={() => onSignalClick(purchase.signalId)}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-foreground mb-1">
                          Signal ID: {purchase.signalId}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {new Date(purchase.timestamp).toLocaleDateString("ar")}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-primary">{purchase.amount} ETH</p>
                        {purchase.quickRating && (
                          <p className="text-xs text-[#00ffcc]">
                            ⭐ {purchase.quickRating}/5
                          </p>
                        )}
                      </div>
                    </div>
